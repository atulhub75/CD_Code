...
public class TS7_Tips_TC1 extends BaseClass{
	...
	public void verifyTipsFunction() throws Exception {
		Log.startTestCase("Test for select Exam JEE Main,sub=Physics and select Zero question");
		...
		tipsHomePage.SelectQuestion("0");
		Log.info("select number of questions");
		tipsHomePage.clickSearchButton();
		Log.info("click search button");
		Thread.sleep(3000);
		boolean result=tipsHomePage.isDisplayQuetionError();
		Assert.assertTrue(result);
		Log.info("validate Question displayed");
		Log.endTestCase("TS7_Tips_TC1");
	}
}

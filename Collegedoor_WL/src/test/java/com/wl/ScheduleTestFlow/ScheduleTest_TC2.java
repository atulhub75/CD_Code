...
public class ScheduleTest_TC2 extends BaseClass{
	...
	public void verifySchedulePaperFunctionality() throws Throwable {
		Log.startTestCase("Test for select Exam and schedule it and edit schedule Exam");
		...
		scheduleTestPage=scheduleTestDetailPage.clickEditScheduleTest();
		Log.info("Click Edit schedule Exam button");
		Thread.sleep(3000);
		scheduleTestPage.clickSubmitButton();
		Log.info("Click submit button");
		scheduleTestDetailPage=scheduleTestPage.clickMessage();
		Log.info("Click successfull schedule message");
		boolean result=scheduleTestDetailPage.validateMessage();
